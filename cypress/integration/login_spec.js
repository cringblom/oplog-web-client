describe('Login page', () => {
    ...
    it('should show notification if no password', () => {
        cy.get('[data-cy=username-input]').type('hello@hello.com')
        cy.get('[data-cy=login-button]').click()
        cy.get('.oplog-notification.error').should('exist')
    })
    ...
    it('should redirect to /operations on login-button click', () => {
        cy.get('[data-cy=username-input]').type('a.test.user@test.com')
        cy.get('[data-cy=password-input]').type('testtest{enter}')
